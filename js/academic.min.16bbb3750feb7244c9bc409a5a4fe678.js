...
window.addEventListener("scroll", () => {
    if (window.pageYOffset < "1000") backtotop.style.display = "none";
    else backtotop.style.display = "block";
})
...
;(function ($) {
    ...
    function scrollToAnchor(target) {
        ...
        if ($(target).length) {
            let elementOffset = Math.ceil(
                $(target).offset().top - getNavBarHeight()
            )
    ...
    $('#navbar-main li.nav-item a.nav-link').on('click', function (event) {
        ...
        if (
            ...
            let elementOffset = Math.ceil(
                $(hash).offset().top - getNavBarHeight()
            )
